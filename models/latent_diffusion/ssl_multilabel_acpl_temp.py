...
class LatentSSLPoolingMultilabelACPLTemp(JointLatentDiffusionMultilabel):
    ...
    def on_train_epoch_end(self) -> None:
        ...
        if self.global_step>=self.min_step_for_acpl:
            self.classification_loss_weight = 0.001
            print("SET CL WEIGHT TO 0.001")
    ...
    def acpl_actions_after_training_loop(self):
        ...
        if dist.get_rank() == 0:
            torch.save(self.trainer.anchor, f"{self.trainer.logdir}/anchor{self.acpl_loop}_on_step{self.global_step}.pth.tar")
