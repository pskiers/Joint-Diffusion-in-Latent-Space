...
class ChestXRay_nih_densenet_2perc(torch.utils.data.Dataset):
    def __init__(self, mode='train', training_platform: str = 'local_sano') -> None:
        ...
        if mode in ['val', 'train']:
            split_file = open("datasets/chest_xray_ssl_train_list_2_1.txt", "r") 
            image_list = [l for l in split_file.read().splitlines()]
            split_file.close() 
            ...
            if mode =='val':
                print('[WARNING]val dataset is train - we onnnly have it to avoid errors')
                self.final_image_df = df.reset_index(drop=True)
                ...
                transformList = []
                transformList.append(transforms.RandomResizedCrop(256, (0.2, 1.0)))
