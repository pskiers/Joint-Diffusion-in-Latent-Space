...
def get_cifar10_multi(args, root):
    transform_labeled = transforms.Compose(
        [
            transforms.RandomHorizontalFlip(),
            transforms.RandomCrop(size=32, padding=int(32 * 0.125), padding_mode="reflect"),
    ...
    train_unlabeled_dataset = CIFAR10SSL(
        ...
        train_unlabeled_idxs,
        train=True,
        transform=TransformMultiFixMatch(mean=cifar10_mean, std=cifar10_std, number=10),
    )
...
def get_svhn_supervised(args, root):
    transform_val = transforms.Compose(
        [transforms.ToTensor(), transforms.Normalize(mean=normal_mean, std=normal_std)]
    )
...
def get_stl10(args, root):
    ...
    transform_unlabeled = transforms.Compose(
        [transforms.Resize, TransformFixMatch(mean=normal_mean, std=normal_std)]
    )
    ...
    train_labeled_dataset = datasets.STL10(
        root=root, split="train", transform=transform_labeled, download=True
    )
    train_unlabeled_dataset = datasets.STL10(
        root=root, split="unlabeled", transform=transform_unlabeled, download=True
    )
    test_dataset = datasets.STL10(root=root, split="test", transform=transform_val, download=True)
